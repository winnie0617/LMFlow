...
def build_dataset(model_name, split="train", input_min_text_length=2, input_max_text_length=8):
    ...
    def tokenize(sample):
        sample["chosen_ids"] = tokenizer.encode(sample["chosen"])
        sample["rejected_ids"] = tokenizer.encode(sample["rejected"])
        return sample
...
if __name__ == "__main__":
    ...
    for label, data in d.items():
        ...
        lengths = []
        n = 1000
        for i, chosen_input in enumerate(zip(data["chosen"][:n], data["rejected"][:n])):
            rewards = torch.zeros(k, 1)
            for i, model in enumerate(models):
                model_inputs = tokenizer(chosen_input[0], padding=True, truncation=True, return_tensors="pt")
                model_inputs.to(device)
                model_outputs = model(input_ids=model_inputs["input_ids"], attention_mask=model_inputs["attention_mask"])
                chosen_score = torch.tensor(model_outputs)

                model_inputs = tokenizer(chosen_input[1], padding=True, truncation=True, return_tensors="pt")
                model_inputs.to(device)
                model_outputs = model(input_ids=model_inputs["input_ids"], attention_mask=model_inputs["attention_mask"])
                reject_score = torch.tensor(model_outputs)
        ...
        plt.figure(1)
        plt.hist(means, bins, label=label, alpha=0.5)
